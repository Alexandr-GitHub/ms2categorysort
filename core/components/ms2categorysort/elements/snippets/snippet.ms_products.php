...
/** ms2categorysort: msCategory parents + opt-in per-category menuindex (sortByCategory) */
$ms2csAddon = ms2categorysort_get_service($modx);
$ms2csService = $ms2csAddon->getCategorySortService();
$parentsParam = isset($scriptProperties['parents']) ? (string) $scriptProperties['parents'] : '';
$depthParam = isset($scriptProperties['depth']) ? (int) $scriptProperties['depth'] : 10;
$parentsFilter = $ms2csService->buildParentsWhere($parentsParam, $depthParam);
if (!empty($parentsFilter['sql'])) {
    $where[] = $parentsFilter['sql'];
    $scriptProperties['parents'] = 0;
}

$categoryContext = $ms2csService->resolveCategoryContext($scriptProperties, $modx->resource);
if ($ms2csService->shouldApplyCategorySort($scriptProperties, $categoryContext)) {
    $sortbyRaw = $scriptProperties['sortby'] ?? '';
    if (is_string($sortbyRaw) && $sortbyRaw !== '' && ($sortbyRaw[0] === '{' || $sortbyRaw[0] === '[')) {
        $decoded = json_decode($sortbyRaw, true);
        if (is_array($decoded)) {
            $sortbyRaw = $decoded;
        }
    }
    ...
    $menuindexSort = false;
    if (is_array($sortbyRaw)) {
        $keys = array_keys($sortbyRaw);
        $menuindexSort = isset($keys[0]) && CategorySortRules::sortByContainsMenuindex((string) $keys[0]);
    } elseif (CategorySortRules::sortByContainsMenuindex((string) $sortbyRaw)) {
        $menuindexSort = true;
    }

    if ($menuindexSort) {
        $orderedIds = $ms2csService->getProductIdsInCategoryOrdered((int) $categoryContext);
        if ($orderedIds !== []) {
            $scriptProperties['sortby'] = 'ids';
            $where['msProduct.id:IN'] = $orderedIds;
        }
    }
}
